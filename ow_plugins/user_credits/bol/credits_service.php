...
final class USERCREDITS_BOL_CreditsService
{
     ...
     * @param $accTypeId
     */
    public function checkActionPrice( $actionId, $accTypeId )
    {
        $actionPrice = $this->actionPriceDao->findActionPrice($actionId, $accTypeId);
        ...
        if ( !$actionPrice )
        {
            ...
            $actionPrice->accountTypeId = $accTypeId;
            $actionPrice->actionId = $actionId;
            $actionPrice->amount = 0;
            $actionPrice->disabled = 0;
        }
    ...
    /**
     * Add action price to account types
     */
    public function addActionPriceToAccountTypes()
    {
        ...
        foreach ( $accTypes as $accType )
        {
            if ( $actions )
            {
                foreach ( $actions as $action )
                {
                    $service->checkActionPrice($action->id, $accType->id);
                }
            }
        }
    }
